...
const [showSuccessPopup, setShowSuccessPopup] = useState(false);
  ...
  const addToCart = () => {
    if (selectedMenuItem) {
      ...
      if (existingItemIndex !== -1) {
        // If the same item with the same special request exists, update quantity
  ...
  const handleConfirmOrder = () => {
    ...
    // Reset cart after order is confirmed
    setTimeout(() => {
      setCartItems([]);
    }, 500);
  };
  ...
  const navigateToReservations = () => {
    ...
    router.push('/my-reservations'); // Adjust this path as needed
  };
    ...
    {cartItems.length === 0 ? (
      ...
      <div className="space-y-4">
        {cartItems.map((item, index) => (
          <div key={`${item._id}-${index}`} className="border-b pb-4">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-semibold">{item.name}</h4>
                <p className="text-gray-600 text-sm">{item.specialRequests}</p>
              </div>
              <button 
        ...
        <div className="border-t pt-4">
          ...
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded mt-4 transition duration-300"
          >
            Confirm Order
          </button>
        </div>
